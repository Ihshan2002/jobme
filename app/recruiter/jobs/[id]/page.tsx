...
function StatusBadge({ status }: { status: Applicant["status"] }) {
  const map = {
    applied: {
      label: "Applied",
      class: "bg-blue-50 dark:bg-blue-950/30 text-blue-600 border-blue-200",
    },
    shortlisted: {
      label: "Shortlisted",
      class: "bg-amber-50 dark:bg-amber-950/30 text-amber-600 border-amber-200",
    },
    accepted: {
      label: "Accepted",
      class:
    ...
    rejected: {
      label: "Rejected",
      class: "bg-red-50 dark:bg-red-950/30 text-red-600 border-red-200",
    },
  };
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">
            {job?.title} — {job?.company_name} · {applicants.length} applicant
        ...
        {applicants.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-800 p-12 text-center">
            ...
            <p className="text-sm text-slate-400 mt-1">
              Share your job to get more visibility
            </p>
          </div>
        ) : (
